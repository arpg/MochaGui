...
int GLUIPainter::getTextLineWidth(const char * text) const
{
    ...
    while(*text != '\0' && *text != '\n')
    {
        w += glutBitmapWidth( GLUT_BITMAP_HELVETICA_10, *text);
        text++;
    }
    w += 2;
    return w;
...
int GLUIPainter::getTextSize(const char * text, int& nbLines) const
{
    ...
    while(*text != '\0')
    {
        if (*text != '\n')
        {
            wLine += glutBitmapWidth( GLUT_BITMAP_HELVETICA_10, *text);
        }
        else
        {
            nbLines++;
...
void GLUIPainter::drawFrame(const Rect & r, int margin, int /*style*/)
{
    //drawRoundedRectOutline(Rect(r.x - margin, r.y - margin, r.w + 2*margin, r.h + 2*margin), nv::Point(margin, margin), cOutline);
                //drawRoundedRect(Rect(r.x - margin, r.y - margin, r.w + 2*margin, r.h + 2*margin), nv::Point(std::min(margin,1), std::min(margin,1)),cSemiTranslucent, cOutline);
                //drawRoundedRect(r,nv::Point(1,1),cTranslucent,cOutline);
                drawRoundedRectOutline(r,nv::Point(1,1),cOutline);
}
...
void GLUIPainter::drawLabel(const Rect & r, const char * text, const Rect & rt, const int& nbLines, bool /*isHover*/, int style,float red, float green, float blue, float alpha)
{
    if (style > 0 )
        drawFrame( r, Point( rt.x, rt.y ), false, false, false );
    drawText( Rect(r.x+rt.x, r.y+rt.y, rt.w, rt.h), text, nbLines,-1,false,false,false,red,green,blue,alpha);
}
...
Rect GLUIPainter::getProgressBarRect(const Rect & r, Rect& rs, float v, Rect& rc) const
{
    Rect rect = r;
...
void GLUIPainter::drawProgressBar(const Rect & r, Rect& rs, float vNormalized, float value, Rect& rc, bool isHover, int style)
{
    int sliderHeight = rs.h;
    int radius = 2;
    Rect frameRect(r.x + rs.x, r.y + rs.y, r.w - 2*rs.x, sliderHeight);
    drawFrame( frameRect, Point(radius,radius), isHover, false, false );
    drawFrame( Rect(r.x + rs.x , r.y + rc.y, rc.w + rc.x, rc.h), Point(std::min(radius,rc.w + rc.x),std::min(rc.w + rc.x,radius)), isHover, true, false );
    char num[50];
    sprintf(num,"%.2F",value);
    int width = getTextLineWidthAt(num,strlen(num));
    drawText( Rect(frameRect.x + frameRect.w/2 - width/2, frameRect.y, width, frameRect.h), num, 1,-1,false,false,false,255,255,255,255);
}
...
void GLUIPainter::init()
{
    ...
    if (m_textListBase == 0)
    {
        ...
        for (int ii = 0; ii < 128; ii++) {
            ...
            glutBitmapCharacter( GLUT_BITMAP_HELVETICA_10, ii);
            glEndList();
        }
    }
}
...
void GLUIPainter::drawText( const Rect& r, const char * text, int nbLines, int caretPos, bool isHover, bool isOn, bool /*isFocus*/ , float red, float green, float blue, float alpha)
{
    ...
    float fontColors[4] = {red, green, blue, alpha};
    glColor4fv(fontColors);
