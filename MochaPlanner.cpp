...
void MochaPlanner::Init()
{
    ...
    m_nh.param("collision_weight", m_dPointWeight(7), m_dPointWeight(7));
    ...
    if (m_bSubToVehicleWp) {
        ...
        m_subVehicleWp = m_nh.subscribe("planner/vehicle_waypoint", 1, &MochaPlanner::vehicleWpCb, this);
    } else {
    ...
    m_subGoalWp = m_nh.subscribe("planner/goal_waypoint", 1, &MochaPlanner::goalWpCb, this);

    m_subWaypoints = m_nh.subscribe("planner/input_waypoints", 1, &MochaPlanner::waypointsCb, this);
    
    m_pubPlan = m_nh.advertise<carplanner_msgs::MotionPlan>("planner/plan",1);

    m_pubWaypoints = m_nh.advertise<geometry_msgs::PoseArray>("planner/waypoints",1);
    m_pubSimPath = m_nh.advertise<nav_msgs::Path>("planner/sim_traj", 1);
    m_pubActualTraj = m_nh.advertise<nav_msgs::Path>("planner/actual_traj", 1);
    m_pubControlTraj = m_nh.advertise<nav_msgs::Path>("planner/control_traj", 1);

    m_pubPotentialPaths = m_nh.advertise<visualization_msgs::MarkerArray>("planner/potential_paths",1);
    m_pubPotentialNorms = m_nh.advertise<visualization_msgs::MarkerArray>("planner/potential_path_norms",1);

    // m_actionApplyVelocities_client.waitForServer();

    // m_actionGetControlDelay_client = new actionlib::SimpleActionClient<carplanner_msgs::GetControlDelayAction>("vehicle/manager/get_control_delay",true);
    // m_actionGetControlDelay_client->waitForServer();

    // m_actionGetInertiaTensor_client = new actionlib::SimpleActionClient<carplanner_msgs::GetInertiaTensorAction>("vehicle/manager/get_inertia_tensor",true);
    // m_actionGetInertiaTensor_client.waitForServer();

    // // m_actionSetNoDelay_client = new actionlib::SimpleActionClient<carplanner_msgs::SetNoDelayAction>("vehicle/manager/set_no_delay",true);
...
void MochaPlanner::dynReconfigCb(carplanner_msgs::MochaPlannerConfig &config, uint32_t level)
{
    ...
    m_dPointWeight(7) = config.collision_weight;
    m_dEps = config.eps;
...
//     result << pt[0], pt[1], dGoalPose[2], rpg::AngleWrap( dGoalPose[3] - problem.m_dStartPose[3] ), dGoalPose[4], dGoalPose[5];  // x_transformed y_transformed z t_clamped c v
//     return result;
// }
...
//         vFunctors[plusIdx] = std::make_shared<mochapc::ApplyCommandsThreadFunctor>(*vCubicProblems[plusIdx],
//                                                                           plusIdx,
//                                                                           pPoses[plusIdx],
//                                                                           errors[plusIdx],
...
//             vFunctors[minusIdx] = std::make_shared<mochapc::ApplyCommandsThreadFunctor>(*vCubicProblems[minusIdx],
//                                                                                minusIdx,
//                                                                                pPoses[minusIdx],
//                                                                                errors[minusIdx],
...
//     actionlib::SimpleActionClient<carplanner_msgs::ApplyVelocitiesAction> actionApplyVelocities_client("planner/apply_velocities"+std::to_string(nWorldId),true);
...
//     problem.m_dTinv = rpg::TInv( rpg::Cart2T( problem.m_dStartPose[0], problem.m_dStartPose[1], problem.m_dStartPose[3] ));  // x y t
//     Eigen::Vector6d dTemp = _TransformGoalPose(dStartPose2D,problem); // x y z t c v
//     problem.m_BoundaryProblem.m_dStartPose = Eigen::Vector4d(dTemp[0],dTemp[1],dTemp[3],dTemp[4]); // x y t c
...
//     problem.m_CurrentSolution.m_dOptParams.head(3) = problem.m_BoundaryProblem.m_dGoalPose.head(3); //  x y t 
        ...
        bool only2d /*= false*/)
{
    ...
    if(only2d == false) {
        if( m_bIterateGN == true ) // planner on && sim off
        {
            ...
            problem.SimulateTrajectory(sample,0,true); // recalc using best solution
            // ROS_INFO("world %d norm %f", 0, problem._CalculateErrorNorm(problem._CalculateSampleError(sample)));
            ...
            {
                m_vPotentialNorms.push_back(problem._CalculateErrorNorm(problem._CalculateSampleError(m_vPotentialPaths[i])));
            }
        ...
        else
        {
            ...
            if(problem.m_bInertialControlActive)
            {
                problem.CalculateTorqueCoefficients(sample);
                problem.SimulateTrajectory(sample,0,true);
            }
        ...
        // sample = problem.m_pBestSolution->m_Sample;
...
void MochaPlanner::pubPotentialPathsViz()
{
    ...
    for (uint i=0; i<m_vPotentialPaths.size(); i++)
    {
        ...
        normviz_msg.markers[i].type = visualization_msgs::Marker::TEXT_VIEW_FACING;
        normviz_msg.markers[i].pose.position.x = m_vPotentialPaths[i].m_vStates[m_vPotentialPaths[i].m_vStates.size()-1].m_dTwv.translation().x();
        normviz_msg.markers[i].pose.position.y = m_vPotentialPaths[i].m_vStates[m_vPotentialPaths[i].m_vStates.size()-1].m_dTwv.translation().y();
        normviz_msg.markers[i].pose.position.z = m_vPotentialPaths[i].m_vStates[m_vPotentialPaths[i].m_vStates.size()-1].m_dTwv.translation().z();
        normviz_msg.markers[i].pose.position.x += offset[0];
...
void MochaPlanner::_pubPlan(std::vector<MotionSample>& path_in)
{   
    if (path_in.empty()) { return; }
    std::vector<MotionSample> some_path = path_in;

    carplanner_msgs::MotionPlan plan_msg;
    plan_msg.header.stamp = ros::Time::now();
    plan_msg.header.frame_id = m_config.map_frame;
    for (std::vector<MotionSample>::iterator segiter=some_path.begin(), end=some_path.end(); segiter!=end; segiter++)
    {
        plan_msg.samples.push_back(segiter->toROS());
    }
...
void MochaPlanner::_pubWaypoints(std::vector<Waypoint*>& pts)
{
    ...
    pts_msg.header.frame_id = m_config.map_frame;
...
{   
    ...
    // convertSomePath2PathArrayMsg(some_path, &patharr_msg, m_config.map_frame);
