...
// #include <xmmintrin.h> // only available on x86
#if defined(_MSC_VER)
...
void DoFusion()
{
    ...
    g_fusion.SetCalibrationPose(Sophus::SE3d(fusion::Cart2T(T_ic)));
            ...
            if(time >= localizerMsg.systemtime()){
                ...
                if(m_bLocalizerActive && globalIndex >= g_nGlobalSkip){
                    ...
                    for( currentParam = g_fusion.m_lParams.begin() ; currentParam != g_fusion.m_lParams.end() ; currentParam++ )
                    {
                        ...
                        //vParamAxes[count*2]->SetPose(fusion::Cart2T((*currentParam).m_dPose));
                        ...
                        vParamAxes[count*2 + 1]->SetPose(fusion::Cart2T(pose));
