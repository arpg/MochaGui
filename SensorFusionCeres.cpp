...
*/
    ...
    {
        m_CurrentPose.m_dPose = data.m_dPose; //m_lParams.back().m_dPose;
        m_CurrentPose.m_dV = data.m_dV; //m_lParams.back().m_dV;
        m_CurrentPose.m_dW = data.m_dW; //m_lParams.back().m_dW;
        m_CurrentPose.m_dTime = data.m_dTime; //m_lParams.back().m_dTime;
    }
    ...
    /*if(m_bTimeCalibrated == false){
        if(m_lGlobalPoses.size() < REQUIRED_TIME_SAMPLES){
            ...
            //std::cout << "Updating: " << data.m_dPose.matrix() << std::endl;
        }else{
            //do time calibration
            _CalibrateTime();
            m_lImuData.clear();
